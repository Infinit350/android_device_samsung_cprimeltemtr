...
LINEEND=" \\"
COUNT=`wc -l ../cprimeltemtr/proprietary-files.txt | awk {'print $1'}`
DISM=`egrep -c '(^#|^$)' ../cprimeltemtr/proprietary-files.txt`
COUNT=`expr $COUNT - $DISM`
for FILE in `egrep -v '(^#|^$)' ../cprimeltemtr/proprietary-files.txt`; do
  COUNT=`expr $COUNT - 1`
  if [ $COUNT = "0" ]; then
    LINEEND=""
  fi
...
ifneq (\$(filter cprimeltemtr,\$(TARGET_DEVICE)),)
